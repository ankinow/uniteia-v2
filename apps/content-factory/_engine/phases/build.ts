...
export interface BuildDeps { llmFn: LLMFn }
...
export interface BuildOk { status: "ok"; core_path: string; retries: number }
export interface BuildFail { status: "fail"; failed_rules: string[]; retries: number; last_yaml?: string }
...
export async function runBuild(input: BuildIn, deps: BuildDeps): Promise<BuildOut> {
  ...
  const basePrompt = tpl({
    id: input.id, entity: input.entity, type: input.type ?? "concept",
    lang: input.lang, extracts,
  })
  ...
  while (attempt <= MAX_RETRIES) {
    ...
    lastYaml = llmOut
    let parsed: unknown
    try { parsed = yaml.parse(stripFences(llmOut)) }
    catch (e) { lastErrors = [{ rule: "yaml:parse", message: (e as Error).message }]; attempt++; continue }

    const schemaRes = validateCoreObject(parsed)
    if (!schemaRes.ok) { lastErrors = schemaRes.errors; attempt++; continue }
    ...
    if (!evRes.ok) { lastErrors = evRes.errors; attempt++; continue }
    return { status: "ok", core_path: corePath, retries: attempt }
  }
  return { status: "fail", failed_rules: lastErrors.map((e) => e.rule), retries: attempt, last_yaml: lastYaml }
}
