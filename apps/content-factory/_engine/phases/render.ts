...
export interface RenderOk { status: "ok"; written: string[] }
export interface RenderFail { status: "fail"; failed_rules: string[] }
export type RenderOut = RenderOk | RenderFail

const FILE_FOR: Record<Channel, string> = {
  blog: "blog.md", short: "short.json", wiki: "wiki.md", "prompt-seed": "prompt-seed.md",
}
const TPL_FOR: Record<Channel, string> = {
  blog: "blog.md.hbs", short: "short.json.hbs", wiki: "wiki.md.hbs", "prompt-seed": "prompt-seed.md.hbs",
}
...
export async function runRender(input: RenderIn): Promise<RenderOut> {
  ...
  try { core = yaml.parse(await fs.readFile(input.corePath, "utf8")) }
  catch { return { status: "fail", failed_rules: ["render:core_missing_or_invalid"] } }
