...
export interface GatherDeps { fetcherFn: FetcherFn }
...
export async function runGather(input: GatherIn, deps: GatherDeps): Promise<GatherOut> {
  ...
  for (const url of input.seed_urls) {
    try { fetched.push(await deps.fetcherFn(url)) } catch { /* gate decides */ }
  }
  ...
  await fs.writeFile(sourcesPath, JSON.stringify(
    fetched.map((f, i) => ({
      ...
      url: f.url, kind: f.kind, accessed_at: f.fetched_at, status: f.status,
    })), null, 2,
  ))
  await fs.writeFile(extractsPath, JSON.stringify(
    fetched.map((f, i) => ({
      excerpt_id: `ex_${String(i + 1).padStart(3, "0")}`,
      url: f.url, text: f.text.slice(0, 12000),
    })), null, 2,
  ))

  if (failed_rules.length > 0) {
    return {
      status: "fail", failed_rules,
