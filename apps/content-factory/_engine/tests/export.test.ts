...
beforeAll(async () => {
  ...
  await fs.copyFile(
    path.join(GOLDEN_DIR, "core.yaml"),
    path.join(workdir, "core.yaml"),
  )
...
describe("export", () => {
  it("produces llm-wiki-uniteia/en/llm-agents-primer.md", async () => {
    const r = await runExport({
      slug: "llm-agents-primer", lang: "en",
      workdir, publishDir: publish, templatesDir: TEMPLATES,
    })
  ...
  it("is idempotent", async () => {
    const first = await fs.readFile(
      path.join(publish, "en", "llm-agents-primer.md"), "utf8",
    )
    await runExport({
      slug: "llm-agents-primer", lang: "en",
      workdir, publishDir: publish, templatesDir: TEMPLATES,
  ...
  it("fails when blog.md is missing", async () => {
    ...
    await fs.copyFile(
      path.join(GOLDEN_DIR, "core.yaml"),
      path.join(empty, "core.yaml"),
    )
    const r = await runExport({
      slug: "llm-agents-primer", lang: "en",
      workdir: empty, publishDir: publish, templatesDir: TEMPLATES,
    })
