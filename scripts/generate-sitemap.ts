...
async function generate() {
  ...
  try {
    ...
    for (const nodeId of contentGraphData.indexes.sitemapEligible) {
      ...
      const loc = `${origin}/${node.locale}/${node.slug}`
      ...
      const hreflangLines = group
        ...
        .map(
          v =>
            `    <xhtml:link rel="alternate" hreflang="${v.lang}" href="${origin}/${v.lang}/${v.slug}" />`
        )
      ...
      if (hasEn) {
        ...
        hreflangLines.push(
          `    <xhtml:link rel="alternate" hreflang="x-default" href="${origin}/en/${enSlug}" />`
        )
      }
