...
function checkContractInvariants(): GateResult[] {
  ...
  try {
    ...
    if (schemas.includes("z.literal(CONTENT_GRAPH_SCHEMA_VERSION)")) {
      ...
      results.push({ gate: "G3a:z-literal-regression", passed: true, evidence: "z.enum active, no z.literal found" })
    }
  } catch {
    results.push({ gate: "G3a:z-literal-regression", passed: false, evidence: `${schemasPath} not readable` })
  }
  ...
  try {
    ...
    if (mw.includes("publishable ? 95 : 65")) {
      ...
      results.push({ gate: "G3b:binary-fallback", passed: true, evidence: "overallScore gate active (50 fallback)" })
    }
  } catch {
    results.push({ gate: "G3b:binary-fallback", passed: false, evidence: `${manifestWriter} not readable` })
  }
  ...
  try {
    ...
    if (localDef && !sharedImport) {
      ...
      results.push({ gate: "G3c:visualasset-dual-def", passed: true, evidence: sharedImport ? "imported from shared" : "no local def" })
    }
  } catch {
    results.push({ gate: "G3c:visualasset-dual-def", passed: false, evidence: `${v2Node} not readable` })
  }
  ...
  results.push({
    ...
    evidence: glassFindings.length === 0 ? "0 blur instances in CSS source" : glassFindings.join("; "),
  })
...
function checkContentGraphDivergence(): GateResult {
  ...
  try {
    ...
    return { gate: "G2:quality-divergence", passed: true, evidence: "content-graph.generated.ts not found (pre-build state)" }
  }
}
