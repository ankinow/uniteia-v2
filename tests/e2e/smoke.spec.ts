...
test('article page renders at /en/article/test-article', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="article-frame"], [data-testid="editorial-verdict"]').first()).toBeVisible({ timeout: 10_000 })
  await page.waitForLoadState('networkidle')
 ...
 * ─────────────────────────────────────────────── */
test('language switcher changes language', async ({ page, context }) => {
  await page.goto('/en')
