...
import { type ToolPackCliOptions, resolveToolPackConfig } from './config.js'
import {
...
function printLogs(
  result: Awaited<ReturnType<typeof readPackedMacLogs>>,
  options: CliOptions
): void {
...
addBuildOptions(
  addSharedOptions(
    cli.command(
      'mac <action>',
      'Mac packaging commands: build|install|start|stop|logs|uninstall|cleanup'
    )
  )
