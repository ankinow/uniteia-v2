...
async function copyResourceTree(config: ToolPackConfig, paths: MacPaths): Promise<void> {
  ...
  await cp(
    join(config.workspaceRoot, 'design-systems'),
    join(paths.resourceRoot, 'design-systems'),
    {
      recursive: true,
    }
  )
...
async function writeAssembledApp(
  ...
  const dependencies = Object.fromEntries(
    INTERNAL_PACKAGES.map(packageInfo => {
      ...
      return [
        packageInfo.name,
        `file:${relative(paths.assembledAppRoot, join(paths.tarballsRoot, tarball))}`,
      ]
    })
  )
...
function commandMatchesDesktopMarker(command: string, marker: DesktopRootIdentityMarker): boolean {
  return (
    command.includes(marker.executablePath) ||
    command.includes(macAppExecutablePath(marker.appPath))
  )
}
...
function isUnmanagedDesktopFallback(fallback: DesktopRootIdentityFallback | undefined): boolean {
  return (
    fallback != null &&
    !['marker-matched', 'marker-not-found', 'marker-pid-not-running'].includes(fallback.reason)
  )
}
...
async function waitForDesktopStatus(
  config: ToolPackConfig,
  timeoutMs = 45_000
): Promise<DesktopStatusSnapshot | null> {
  const stamp = desktopStamp(config)
  const startedAt = Date.now()
  while (Date.now() - startedAt < timeoutMs) {
    try {
      return await requestJsonIpc<DesktopStatusSnapshot>(
        stamp.ipc,
...
export async function installPackedMacDmg(config: ToolPackConfig): Promise<MacInstallResult> {
  ...
  try {
    ...
    await execFileAsync('ditto', [
      join(paths.mountPoint, `${PRODUCT_NAME}.app`),
...
async function findManagedDesktopProcessTree(config: ToolPackConfig): Promise<{
  ...
  const pids = collectProcessTreePids(processes, [...stampedRootPids, identity.rootPid])
...
export async function stopPackedMacApp(config: ToolPackConfig): Promise<MacStopResult> {
  ...
  const remainingAfterGraceful = gracefulRequested
    ? await waitForNoManagedDesktopProcesses(config)
    : before
  if (remainingAfterGraceful.pids.length === 0) {
    const unmanaged =
      !gracefulRequested && before.pids.length === 0 && isUnmanagedDesktopFallback(before.fallback)
    if (!unmanaged) {
  ...
  return {
    ...(remainingAfterGraceful.fallback == null
      ? {}
      : { fallback: remainingAfterGraceful.fallback }),
    gracefulRequested,
    namespace: config.namespace,
