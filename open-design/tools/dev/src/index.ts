...
import { type FileHandle, lstat, mkdir, open, rm, symlink, writeFile } from 'node:fs/promises'
...
function printStatusResult(
  ...
  if (apps != null) {
    ...
    const details = [namespace == null ? null : `namespace ${namespace}`, statusLabel].filter(
...
function printRunForegroundResult(
  ...
  if (webUrl != null || daemonUrl != null) {
    ...
    if (webUrl != null)
      process.stdout.write(`  ➜  Web:    ${colorizeLink(normalizeDisplayUrl(webUrl))}\n`)
    if (daemonUrl != null)
      process.stdout.write(`  ➜  Daemon: ${colorizeLink(normalizeDisplayUrl(daemonUrl))}\n`)
...
async function status(config: ToolDevConfig, appName: string | undefined) {
  ...
  const apps = Object.fromEntries(
    await Promise.all(
      targets.map(async target => [target, await inspectAppStatus(config, target)] as const)
    )
...
function printLogs(result: LogResult | Record<string, LogResult>, options: CliOptions) {
  ...
  for (const [appName, entry] of entries) {
    ...
    process.stdout.write(
      entry.lines.length > 0 ? `${entry.lines.join('\n')}\n` : '(no log lines)\n'
    )
  }
}
    ...
    const entries = Object.entries(diagnostics)
      .map(
        ([appName, value]) =>
          [
            appName,
            Array.isArray(value) ? formatLogDiagnostics(value as LogDiagnostic[]) : null,
          ] as const
      )
...
async function runForeground(
  ...
  const started = await runSequential(targets, target =>
    startApp(config, target, foregroundOptions)
  )
...
addPortOptions(
  addSharedOptions(
    cli.command('start [app]', 'Start daemon, web, desktop, or all when app is omitted')
  )
...
addPortOptions(
  addSharedOptions(
    cli.command('run [app]', 'Start apps and keep this command alive until interrupted')
  )
).action(async (appName: string | undefined, options: CliOptions) => {
...
addSharedOptions(
  cli.command('status [app]', 'Show app status for daemon, web, desktop, or all')
).action(async (appName: string | undefined, options: CliOptions) => {
  printStatusResult(await status(resolveToolDevConfig(options), appName), options, appName)
})

addSharedOptions(
  cli.command('stop [app]', 'Stop daemon, web, desktop, or all when app is omitted')
).action(async (appName: string | undefined, options: CliOptions) => {
  const config = resolveToolDevConfig(options)
...
addPortOptions(
  addSharedOptions(
    cli.command('restart [app]', 'Restart daemon, web, desktop, or all when app is omitted')
  )
).action(async (appName: string | undefined, options: CliOptions) => {
...
addSharedOptions(
  ...
  const result =
    targets.length === 1
      ...
      : Object.fromEntries(
          await Promise.all(
            targets.map(async target => [target, await readLogs(config, target)] as const)
          )
        )
...
addSharedOptions(
  cli.command('check [app]', 'Print status and recent logs for quick diagnostics')
).action(async (appName: string | undefined, options: CliOptions) => {
  const config = resolveToolDevConfig(options)
  const targets = resolveTargetApps(appName, DEFAULT_START_APPS)
  const apps = Object.fromEntries(
  ...
  printCheckResult(
    { apps, diagnostics: createLogDiagnostics(logs), logs, namespace: config.namespace },
    options
  )
})
...
if (
  cliArgs.length === 0 ||
  (cliArgs[0]?.startsWith('-') && cliArgs[0] !== '--help' && cliArgs[0] !== '-h')
) {
