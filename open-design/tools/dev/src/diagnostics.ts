...
const NATIVE_ADDON_ABI_MISMATCH_PATTERN =
  /was compiled against a different Node\.js version[\s\S]*?NODE_MODULE_VERSION\s+\d+[\s\S]*?requires\s+NODE_MODULE_VERSION\s+\d+/i
...
export function detectLogDiagnostics(lines: readonly string[]): LogDiagnostic[] {
  ...
  if (
    !NATIVE_ADDON_ABI_MISMATCH_PATTERN.test(logText) &&
    !NODE_MODULE_VERSION_PATTERN.test(logText)
  )
    return []
...
export function createStartupLogDiagnostics(
  logPath: string,
...
export function appendStartupLogDiagnostics(
  error: unknown,
  appName: string,
  details: StartupLogDiagnostics
): Error {
