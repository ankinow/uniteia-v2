...
function resolveAppConfig(options: {
  ...
  return {
    ...
    latestLogPath: resolveLogFilePath({
...
export function resolveTargetApps(
  ...
  defaults: readonly ToolDevAppName[]
): ToolDevAppName[] {
...
export function parsePortOption(
...
export function resolveToolDevConfig(options: ToolDevOptions = {}): ToolDevConfig {
  ...
  const toolsDevRoot = resolveSidecarBase({
    base:
      options.toolsDevRoot ??
      process.env[SIDECAR_ENV.BASE] ??
      resolveSourceRuntimeRoot({
        contract: OPEN_DESIGN_SIDECAR_CONTRACT,
  ...
  return {
    apps: {
      ...
      web: {
        ...web,
        nextDistDir: resolveAppRuntimePath({
          app: APP_KEYS.WEB,
          namespaceRoot,
          fileName: 'next',
          contract: OPEN_DESIGN_SIDECAR_CONTRACT,
        }),
        nextTsconfigPath: resolveAppRuntimePath({
          app: APP_KEYS.WEB,
          namespaceRoot,
