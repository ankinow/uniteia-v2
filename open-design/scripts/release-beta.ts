...
function extractStableVersion(release: GitHubRelease): ParsedStableVersion | null {
  const candidates = [release.tag_name, release.name].filter(
...
function extractBetaVersion(release: GitHubRelease): ParsedBetaVersion | null {
  const tagMatch =
    typeof release.tag_name === 'string' ? betaTagPattern.exec(release.tag_name) : null
  if (tagMatch?.[1] != null && tagMatch[2] != null) {
    return parseBetaParts(tagMatch[1], tagMatch[2])
  }

  const candidates = [release.name, release.body].filter(
    (value): value is string => typeof value === 'string'
  )
  for (const candidate of candidates) {
...
async function readPackagedVersion(): Promise<string> {
  ...
  if (!stableVersionPattern.test(packageJson.version)) {
    fail(
      `apps/packaged/package.json version must be a stable x.y.z base version; got ${packageJson.version}`
    )
  }
...
const packagedParsed =
  parseStableVersion(packagedVersion) ?? fail(`invalid packaged version: ${packagedVersion}`)
...
if (latestStable != null && compareVersions(packagedParsed, latestStable.parsed) <= 0) {
  fail(
    `packaged base version ${packagedVersion} must be strictly greater than latest stable ${latestStable.value}`
  )
}
...
if (latestMacAsset?.id != null) {
  const beta = extractBetaVersionFromLatestMacYml(
    await fetchReleaseAssetText(repository, latestMacAsset.id)
  )
  if (beta != null) betaCandidates.push(beta)
}
...
for (const beta of betaCandidates) {
  ...
  if (ordering < 0) {
    fail(
      `packaged base version ${packagedVersion} regressed below current beta base version ${beta.baseVersion}`
    )
  }
