...
test.before(async () => {
  ...
  ;({ startServer } = (await import('../../apps/daemon/dist/server.js')) as {
...
test(
  'runtime adapter live run flow streams a successful response for every available runtime',
  { timeout: liveTimeoutMs * maxRuntimeCount + 30_000 },
  async () => {
    if (!detectedAgents) {
...
async function runRuntime(agent: AgentInfo): Promise<void> {
  ...
  log(
    'run',
    `${agent.id}: end event code=${String(end.data.code)} signal=${String(end.data.signal ?? 'none')}`
  )
  ...
  const text = events
    .map(event => {
      ...
      if (event.event === 'agent')
        return stringData(event.data.text) || stringData(event.data.delta)
      return ''
    })
    .join('')
