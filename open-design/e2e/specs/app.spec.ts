...
async function createProject(page: Parameters<typeof test>[0]['page'], entry: UICase) {
...
async function sendPrompt(page: Parameters<typeof test>[0]['page'], prompt: string) {
...
async function runExampleUsePromptFlow(page: Parameters<typeof test>[0]['page'], entry: UICase) {
...
async function runQuestionFormSubmitPersistenceFlow(
  ...
  await toneQuestion
    .locator('label.qf-chip', { has: page.getByText('Editorial / magazine') })
    .click()
  ...
  await expect(
    form.getByText('Answers sent — agent is using these for the rest of the session.')
  ).toBeVisible()
  ...
  const formAnswerMessage = messages.find(
    message => message.role === 'user' && message.content.includes('[form answers — discovery]')
  )
...
async function createProjectNameOnly(page: Parameters<typeof test>[0]['page'], entry: UICase) {
...
async function expectArtifactVisible(page: Parameters<typeof test>[0]['page'], entry: UICase) {
...
async function runFileMentionFlow(page: Parameters<typeof test>[0]['page'], entry: UICase) {
  const current = new URL(page.url())
  ...
  await page
    ...
    .getByRole('button', { name: /reference\.txt/i })
    .click()
...
async function runDeepLinkPreviewFlow(page: Parameters<typeof test>[0]['page'], entry: UICase) {
  await sendPrompt(page, entry.prompt)
  await expectArtifactVisible(page, entry)
...
async function runFileUploadSendFlow(page: Parameters<typeof test>[0]['page'], entry: UICase) {
  ...
  await expect(page.getByText(entry.prompt, { exact: true })).toBeVisible()
  await expect(
    page.locator('.user-attachments').getByText('reference.txt', { exact: true })
  ).toBeVisible()
}

async function runDesignFilesUploadFlow(page: Parameters<typeof test>[0]['page']) {
...
async function runDesignFilesDeleteFlow(page: Parameters<typeof test>[0]['page']) {
...
async function runDesignFilesTabPersistenceFlow(page: Parameters<typeof test>[0]['page']) {
...
async function runConversationDeleteRecoveryFlow(
  ...
  const activeRow = page.getByTestId('conversation-list').locator('.chat-conv-item.active').first()
