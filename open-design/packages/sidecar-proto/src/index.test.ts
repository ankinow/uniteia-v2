...
describe('open-design sidecar contract', () => {
  ...
  it('validates daemon IPC messages', () => {
    expect(normalizeDaemonSidecarMessage({ type: SIDECAR_MESSAGES.STATUS })).toEqual({
      type: 'status',
    })
    expect(normalizeDaemonSidecarMessage({ type: SIDECAR_MESSAGES.SHUTDOWN })).toEqual({
      type: 'shutdown',
    })
    expect(() =>
      normalizeDaemonSidecarMessage({ input: {}, type: SIDECAR_MESSAGES.EVAL })
    ).toThrow()
  })
  ...
  it('validates desktop IPC message inputs', () => {
    ...
    expect(() =>
      normalizeDesktopSidecarMessage({ input: { expression: 42 }, type: SIDECAR_MESSAGES.EVAL })
    ).toThrow()
    expect(() =>
      normalizeDesktopSidecarMessage({ input: { selector: '' }, type: SIDECAR_MESSAGES.CLICK })
    ).toThrow()
  })
})
