...
export function normalizeNamespace(namespace: unknown): string {
  ...
  if (value !== namespace)
    throw new Error('namespace must not contain leading or trailing whitespace')
...
function normalizeMessageType(value: unknown, label: string): string {
  if (typeof value !== 'string' || value.length === 0) {
    throw new SidecarContractError(
      SIDECAR_ERROR_CODES.INVALID_MESSAGE,
      `${label} type must be a non-empty string`
    )
  }
  return value
}
...
export function normalizeDaemonSidecarMessage(input: unknown): DaemonSidecarMessage {
  ...
  throw new SidecarContractError(
    SIDECAR_ERROR_CODES.UNKNOWN_MESSAGE,
    `unknown daemon sidecar message: ${type}`
  )
}
...
export function normalizeWebSidecarMessage(input: unknown): WebSidecarMessage {
  ...
  throw new SidecarContractError(
    SIDECAR_ERROR_CODES.UNKNOWN_MESSAGE,
    `unknown web sidecar message: ${type}`
  )
}
...
export function normalizeDesktopSidecarMessage(input: unknown): DesktopSidecarMessage {
  ...
  switch (type) {
    ...
    default:
      throw new SidecarContractError(
        SIDECAR_ERROR_CODES.UNKNOWN_MESSAGE,
        `unknown desktop sidecar message: ${type}`
      )
  }
}
