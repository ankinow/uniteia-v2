...
import { type Server, createConnection, createServer as createNetServer } from 'node:net'
...
export type RuntimeRootRequest<TStamp extends SidecarStampShape = SidecarStampShape> =
  RuntimePathRequest<TStamp> & {
    runId: string
  }
...
export function normalizeIpcPath(ipc: unknown): string {
  ...
  if (ipc.trim() !== ipc)
    throw new Error('sidecar ipc path must not contain leading or trailing whitespace')
...
export function resolveSidecarBase<TStamp extends SidecarStampShape>({
  ...
  return resolve(
    base ?? env[contract.env.base] ?? resolveSourceRuntimeRoot({ contract, projectRoot, source })
  )
}
...
export function createSidecarLaunchEnv<TStamp extends SidecarStampShape>({
  ...
  return {
    ...extraEnv,
    [contract.env.base]: resolveSidecarBase({
      base,
      contract,
      env: extraEnv,
...
async function allocateForcedPort(
  ...
  try {
    ...
    throw new Error(
      `forced ${label} port ${port} is not available (${errorCode(error) ?? errorMessage(error)})`
    )
  } finally {
...
async function allocateDynamicPort(
  label: string,
  host: string,
  reserved: Set<number>
): Promise<PortAllocation> {
